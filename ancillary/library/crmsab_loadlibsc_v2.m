...
[ Alib,infoAall,valid_idx,Aall ] = crmsab_loadAlibconv(optLibs,wabasename,optInterpid,c,bands_opt,wvc,cntRmvl,varargin{:});
...
%     masterbase = crmsab_const_libmasterbase('CRISMspclib',optCRISMspclib,wa_identfr,optInterp(1).method,optInterp(1).retainRatio);
...
% 
%     % RELAB
%     masterbase = crmsab_const_libmasterbase('RELAB',optRELAB,wa_identfr,optInterp(2).method,optInterp(2).retainRatio);
%     [cachefilepath] = crmsab_const_libcachefilepath(Alibdir,masterbase,c);
%     if ~exist(cachefilepath,'file')
%         % error('%s does not exist.\nPlease perform crism_libConvoluter first.',cachefilepath);
...
%     masterbase = crmsab_const_libmasterbase('USGSsplib',optUSGSsplib,wa_identfr,optInterp(3).method,optInterp(3).retainRatio);
%     [cachefilepath] = crmsab_const_libcachefilepath(Alibdir,masterbase,c);
%     if ~exist(cachefilepath,'file')
%         error('%s does not exist.\nPlease perform crism_libConvoluter first.',cachefilepath);
%     else
...
%     masterbase = crmsab_const_libmasterbase('CRISMTypeLib',optCRISMTypeLib,wa_identfr,optInterp(4).method,optInterp(4).retainRatio);
%     [cachefilepath] = crmsab_const_libcachefilepath(Alibdir,masterbase,c);
%     if ~exist(cachefilepath,'file')
%         error('%s does not exist.\nPlease perform crism_libConvoluter first.',cachefilepath);
%     else
