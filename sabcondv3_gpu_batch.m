function [Yif_cor,Bg_est,AB_est,T_est] = sabcondv3_gpu_batch(obs_id,varargin)
% huwacb option
nIter = 5;
vis = 0;
lambda_a = 0.01;
...
% Yif_cor = nan([nBall,nLall,nCall]);
% % Yif_isnan = nan([nBall,nLall,nCall]);
% T_est = nan([nBall,nCall]);
% Bg_est = nan([nBall,nLall,nCall]); AB_est = nan([nBall,nLall,nCall]);
...
if strcmpi(precision,'single')
    ...
    logT_extrap = single(logT_extrap);
    WA = single(WA);
end
...
for ni = 1:n_batch
    ...
    = sabcondc_v3l1_gpu_batch(logYif(:,:,cList),WA(:,cList),Alibs,logT_extrap(:,:,cList),...
              ...
              'verbose_huwacb',verbose_huwacb,'nIter',nIter);%,'debug_huwacb',debug_huwacb);
