function [Yif_cor,Bg_est,AB_est,T_est,Yif_cor_nr,Yif_cor_ori,...
    ...
    = sabcondv4_columntest(obs_id,cList,varargin)
% huwacb option
nIter = 5;
vis = 0;
lambda_a = 0.01;
...
Ice_est = nan([nBall,nLall,nCLength]);
for ci=1:nCLength
    c = cList(ci);
    ...
    if ~all(isnan(WA(:,c)))
        ...
        [ logt_est,logYifc_cor,logAB,logBg,logIce,logYifc_cor_ori,logYifc_isnan,ancillary,rr_ori_c,vldpxl_c]...
             = sabcondc_v4l1_b(Alib,Aicelib,logYif(:,:,ci),WA(:,c),logtc,'GP',GP(:,:,c),...
...
%             = sabcondc_v3l1(Alib,logYif(:,:,ci),WA(:,c),logtc,'GP',GP(:,:,c),...
