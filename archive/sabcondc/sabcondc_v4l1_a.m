function [ logt_est,logYifc_cor,logAB,logBg,logYifc_cor_ori,logYifc_isnan,ancillary,rr_ori,vldpxl] = sabcondc_v4l1_a( Alib,Aice,logYifc,wvc,logtc,varargin )
...
%logt_est_ice = logIce_bprmvd(:,vldpxl) * Xlogtc_1d(:,vldpxl)' / (Xlogtc_1d(:,vldpxl)*Xlogtc_1d(:,vldpxl)');

Xice_sum = sum(Xice(:,vldpxl),1);
[vv,ii] = sort(Xice_sum,'descend');
%logt_est_ice = lad_gadmm_a_v2(Xlogtc_1d(:,vldpxl(ii(1:100)))', logIce_bprmvd(:,vldpxl(ii(1:100)))',...
%                                'tol',tol_lad,'maxiter',maxiter_lad,'verbose',verbose_lad);
%logt_est_ice = logt_est_ice';

logt_est_ice = logIce_bprmvd(:,vldpxl(ii(1:end))) * Xlogtc_1d(:,vldpxl(ii(1:end)))' / (Xlogtc_1d(:,vldpxl(ii(1:end)))*Xlogtc_1d(:,vldpxl(ii(1:end)))');
...
% if ~isempty(Aice)
%     idxAice2 = false(size(A_bprmvd,2));
