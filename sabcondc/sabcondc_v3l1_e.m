function [ logt_est,logYifc_cor,logAB,logBg,logYifc_cor_ori,logYifc_isnan,ancillary,rr_ori,vldpxl] = sabcondc_v3l1_e( Alib,logYifc,wvc,logtc,varargin )
...
% eliminate the observation that causes problems
eli_detect = sum(Xlib(eli_bl,:),1)>th_eli;
vldpxl_t = and(vldpxl,~eli_detect);

Xlogtc_1d = sum(Xlogtc,1);

R_bprmvd = logYifc_bprmvd - logBg_bprmvd - logAB_bprmvd;
r_lad = zeros(Ny,L_bprmvd); d_lad = zeros(Ny,L_bprmvd); Rhov_lad = ones(Ny,1);
[logt_est_bprmvd,r_lad(vldpxl_t,:),d_lad(vldpxl_t,:),rho_lad,Rhov_lad(vldpxl_t,:)]...
    = lad_gadmm_a_v2(Xlogtc_1d(:,vldpxl_t)', R_bprmvd(:,vldpxl_t)',...
...
eli_bl_A = [false eli_bl]; % index of the library excluded from the estimation of logt_est
...
%             logYifc_bprmvd = interp_nan_column_given(logYifc_bprmvd,logYifc_bprmvd_isnan,logYifc_model_bprmvd);
    ...
    % remove nans
    % eliminate the observation that causes problems
    eli_detect = sum(X(eli_bl_A,:),1)>th_eli;
    vldpxl_t = and(vldpxl,~eli_detect);
    
    %update logt_est!
    R_bprmvd = logYifc_bprmvd - logBg_bprmvd - logAB_bprmvd;
    [logt_est_bprmvd,r_lad(vldpxl_t,:),d_lad(vldpxl_t,:),rho_lad,Rhov_lad(vldpxl_t,:)]...
        = lad_gadmm_a_v2(X(1,vldpxl_t)', R_bprmvd(:,vldpxl_t)',...
        'X0',A_bprmvd(:,1)','R0',r_lad(vldpxl_t,:),'D0',d_lad(vldpxl_t,:),...
        'rho',rho_lad,'Rhov',Rhov_lad(vldpxl_t,:),...
        'tol',tol_lad,'maxiter',maxiter_lad,'verbose',verbose_lad);
    logt_est_bprmvd = logt_est_bprmvd';
