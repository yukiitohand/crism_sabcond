% Aice will be kept until the second iteration
function [ logt_est,logYifc_cor,logAB,logBg,logYifc_cor_ori,logYifc_isnan,ancillary,rr_ori,vldpxl] = sabcondc_v4l1_1_a( Alib,Aice,logYifc,wvc,logtc,varargin )
...
idxAicestrt = size(logtc,2)+1;
idxAlibstrt = size(logtc,2)+size(Aice,2)+1;
...
[logt_est_bprmvd_mol,r_lad(vldpxl,:),d_lad(vldpxl,:),rho_lad,Rhov_lad(vldpxl,:)] = lad_gadmm_a_v2(Xlogtc_1d(:,vldpxl)', R_bprmvd(:,vldpxl)',...
                                'tol',tol_lad,'maxiter',maxiter_lad,'verbose',verbose_lad);
logt_est_bprmvd_mol = logt_est_bprmvd_mol';

Xice_sum = sum(Xice(:,vldpxl),1);
[vv,ii] = sort(Xice_sum,'descend');

r_lad_ice = nan(Ny,L_bprmvd); d_lad_ice = nan(Ny,L_bprmvd); Rhov_lad_ice = ones(Ny,1);
%rr_ice = nan(Ny,L_bprmvd);
% [logt_est_ice]...
%     = lad_gadmm_a_v2(Xlogtc_1d(:,vldpxl(ii(1:30)))', logIce_bprmvd(:,vldpxl(ii(1:30)))',...
%                               'tol',tol_lad,'maxiter',maxiter_lad,'verbose',verbose_lad);
% logt_est_ice = logt_est_ice';
% rr_ice(:,vldpxl) = logIce_bprmvd - logt_est_ice * Xlogtc_1d(:,vldpxl);
logt_est_ice = logIce_bprmvd(:,vldpxl(ii(1:30))) * Xlogtc_1d(:,vldpxl(ii(1:30)))' / (Xlogtc_1d(:,vldpxl(ii(1:30)))*Xlogtc_1d(:,vldpxl(ii(1:30)))');
logt_est_bprmvd = logt_est_bprmvd_mol + logt_est_ice;

resNew_bprmvd = R_bprmvd - logt_est_bprmvd_mol*Xlogtc_1d;
...
if ~isempty(Aice)
    ...
    X = [Xlogtc_1d;zeros(size(Xice));Xlib];
    D1ice = zeros(size(D1(idxAicestrt:idxAlibstrt-1,:)));
    D = [zeros(1,size(D1,2));D1ice;D1(idxAlibstrt:end,:)];
else
...
if ~isempty(Aice)
    idxAice2 = false(1,size(A_bprmvd,2));
    idxAice2(2:(size(Aice,2)+1)) = true;
    idxAlib2 = false(1,size(A_bprmvd,2));
    idxAlib2((size(Aice,2)+2):end) = true;
else
    idxAlib2 = false(size(A_bprmvd,2));
    idxAice2(2:end) = true;
end
...
%     end
    ...
    %update logt_est!
%     R_bprmvd = logYifc_bprmvd - logBg_bprmvd - logAB_bprmvd - logIce_bprmvd;
%     [logt_est_bprmvd_mol,r_lad(vldpxl,:),d_lad(vldpxl,:),rho_lad,Rhov_lad(vldpxl,:)]...
...
%     logt_est_bprmvd_mol = logt_est_bprmvd_mol';
    ...
    Xice_sum = sum(X(idxAice2,vldpxl),1);
    if all(Xice_sum<1e-5)
        break;
    end
    [vv,ii] = sort(Xice_sum,'descend');
...
%         resNew_bprmvd = R_bprmvd - logt_est_bprmvd_mol*X(1,:);
%     else
%         res_bprmvd = R_bprmvd-A_bprmvd(:,1)*X(1,:);
...
%% finally, ice is removed
X_logt_est = X(1,:);
Xlib = X(idxAlib2,:);
A_bprmvd = [logt_est_bprmvd Alib_bprmvd];

X = [X_logt_est;Xlib];
D = [D(1,:); D((2+size(Aice,2)):end,:)];
...
ancillary.huwacb_func = 'huwacbl1_gadmm_a';
