function [logYif_cor,logt_est,logAB,logBg,logIce,logYif_isnan,Xt,Xlib,Xice,badspc]...
    = sabcondc_v3l1_gpu_batch_tagv1_R2017b(logYif,WA,Alib,logT,BP,varargin)
...
% 
% This is the stable version corresponding to v1 series release. This is
% the version compatible to R2017b or earlier. (relating to new input 
% patterns of functions "sum" and "mean".)
% diverged from sabcondc_v3l1_gpu_batch_tagv1.m of the version 41e28db
%
% INPUT PARAMETERS
...
if ffc_mode
    if batch
        [ X,Z,~,D,rho,Rhov,~,~,cost_val]...
            = huwacbl1_admm_gat_a_batch_R2017b(A,logYif-logT,C,'LAMBDA_A',lambda_a_2,...
        ...
        [  X,Z,~,~,D,rho,Rhov,~,~,cost_val]...
            = huwacbl1_admm_gat_a_R2017b(A,logYif-logT,WA,'LAMBDA_A',lambda_a_2,...
    ...
    if batch
        [ X,Z,~,D,rho,Rhov,~,~,cost_val]...
            = huwacbl1_admm_gat_a_batch_R2017b(A,logYif,C,'LAMBDA_A',lambda_a_2,...
                    'LAMBDA_C',lambda_c,'LAMBDA_R',lambda_r,'C2_Z',c2_z,......
                    'tol',1e-5,'maxiter',maxiter_huwacb,'verbose',verbose_huwacb,...
                    'precision',precision,'YNORMALIZE',y_normalize); % toc;
    else
        [  X,Z,~,~,D,rho,Rhov,~,~,cost_val]...
        = huwacbl1_admm_gat_a_R2017b(A,logYif,WA,'LAMBDA_A',lambda_a_2,...
        'LAMBDA_C',lambda_c,'LAMBDA_R',lambda_r,'YNORMALIZE',y_normalize,.....
        'tol',1e-5,'maxiter',maxiter_huwacb,'verbose','no',...
        'precision',precision,'gpu',gpu,'Concavebase',C,'debug',debug_huwacb);
    end
...
resNrm = nansum(nansum(abs(RR).* lambda_r,2),1);
...
if batch
    [logt_est,r_lad,d_lad,rho_lad,Rhov_lad,~,~,cost_val,Kcond]...
           = lad_admm_gat_b_batch_R2017b(permute(Xtc,[2,1,3]), permute(RR,[2,1,3]),...
    ...
    = lad_admm_gat_b_R2017b(permute(Xtc,[2,1,3]), permute(RR,[2,1,3]),...
...
resNewNrm = nansum(nansum(abs(lambda_r .* RR),2),1);
...
for n=2:nIter
    ...
    if n==2
        if ffc_mode
            if batch
                 [ X,Z,~,D,rho,Rhov,~,~,cost_val,Tcond ]...
                    = huwacbl1_admm_gat_a_batch_R2017b(A,logYif-logt_est,C,...
                ...
                [ X,Z,~,~,D,rho,Rhov,~,~,cost_val]...
                = huwacbl1_admm_gat_a_R2017b(A,logYif-logt_est,WA,'LAMBDA_A',lambda_a_2,...
            ...
            if batch
                [ X,Z,~,D,rho,Rhov,~,~,cost_val,Tcond ]...
                    = huwacbl1_admm_gat_a_batch_R2017b(A,logYif,C,...
                        'LAMBDA_A',lambda_a_2,'LAMBDA_C',lambda_c,'LAMBDA_R',lambda_r,...
                        'C2_Z',c2_z,'Z0',Z,'D0',D,'X0',X,'R0',RR,'rho',rho,'Rhov',Rhov,...
                        'verbose',verbose_huwacb,'tol',1e-5,'maxiter',maxiter_huwacb,...
                        'PRECISION',precision,'YNORMALIZE',y_normalize);
        ...
        if ffc_mode
            if batch
                [ X,Z,~,D,rho,Rhov,~,~,cost_val ] = huwacbl1_admm_gat_a_batch_R2017b(A,logYif-logt_est,C,...
                  'LAMBDA_A',lambda_a_2,'LAMBDA_C',lambda_c,'LAMBDA_R',lambda_r,...
                  'C2_Z',c2_z,'Z0',Z,'D0',D,'X0',X,'R0',RR,'rho',rho,'Rhov',Rhov,...
            ...
            if batch
               [ X,Z,~,D,rho,Rhov,~,~,cost_val ] = huwacbl1_admm_gat_a_batch_R2017b(A,logYif,C,...
                  'LAMBDA_A',lambda_a_2,'LAMBDA_C',lambda_c,'LAMBDA_R',lambda_r,...
                  'C2_Z',c2_z,'Z0',Z,'D0',D,'X0',X,'R0',RR,'rho',rho,'Rhov',Rhov,...
                  'verbose',verbose_huwacb,'tol',tol_huwacb,'maxiter',maxiter_huwacb,...
                  'PRECISION',precision,'Tcond',Tcond,'YNORMALIZE',y_normalize);
    ...
    resNrm = nansum(nansum(abs(lambda_r .* RR),2),1);
    ...
    if batch
        [logt_est,r_lad,d_lad,rho_lad,Rhov_lad,~,~,cost_val]...
           = lad_admm_gat_b_batch_R2017b(permute(Xtc,[2,1,3]), permute(RR,[2,1,3]),...% 'rho',rho_lad,'Rhov',Rhov_lad,...
                'lambda_r',permute(lambda_r,[2,1,3]),'tol',tol_lad,'maxiter',maxiter_lad,...
...
if ffc_mode
    if batch
        [ X,Z,D,rho,Rhov,~,~,cost_val ] = huwacbl1_admm_gat_a_batch_R2017b(A,...
            logYif-logt_est,C,...
            'LAMBDA_A',lambda_a_2,'LAMBDA_C',lambda_c,'LAMBDA_R',lambda_r,...
            'C2_Z',c2_z,'Z0',Z,'D0',D,'X0',X,'R0',RR,...%'rho',rho,'Rhov',Rhov,...
            'verbose',verbose_huwacb,'tol',tol_huwacb,'maxiter',maxiter_huwacb,...
...
if batch
    [logYif_cor,logt_est,logAB,logBg,logIce,logYif_isnan,X,Xtc,badspc]...
        = gather(logYif_cor,logt_est,logAB,logBg,logIce,logYif_isnan,X,Xtc,badspc);
else
end
